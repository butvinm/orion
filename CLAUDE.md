...
- **Eager LT pre-encoding at LoadModel (optimize-server-rss branch, measured 2026-05-16, cpu.16.128.240).** Pre-encoding `lintrans.LinearTransformation` data once in `LoadModel` (instead of every `Forward`) trades load-time for forward-time RSS. logn=15 measured deltas (3 boundary-band samples avg): `rss_post_load` 15.7 GB → 43.6 GB (+27.9 GB resident), `rss_post_forward1` 27.3 GB → 45.7 GB (+18.4 GB; Forward no longer spikes), `VmHWM` 55.9 GB → 49.2 GB (**~6.7 GB / 12% peak savings**). Load wall-clock 6.6 s → 136.3 s (~20× slower; the encode cost moves from per-Forward into LoadModel exactly once). **At logn=16 the change net-regresses: `bench keygen` is OOM-killed at 130 GB during eager LT encoding** because `bench keygen` calls full `LoadModel` rather than a lightweight `ParseClientParams` path. Baseline logn=16 forward peak is ~117 GB (VmHWM); pre-encoding pushes load-time RSS past the 128 GB ceiling. Fix is either a `keygen`-only metadata parse that skips LT encoding, or streaming/external-buffer support inside `Encode`.
