...
class LevelDAG(nx.DiGraph):
    ...
    def __add__(self, other):
        ...
        aggregate_level_dag = LevelDAG(l_eff=self.l_eff, network_dag=self.network_dag, path=None)
        aggregate_level_dag.add_nodes_from((node, {"weight": 0}) for node in source_nodes)
        aggregate_level_dag.add_nodes_from((node, {"weight": 0}) for node in target_nodes)
        ...
        for source in source_nodes:
            for target in target_nodes:
                ...
                if length1 != float("inf") and length2 != float("inf"):
                    for path, dag in zip([path1, path2], [self, other], strict=False):
                ...
                aggregate_level_dag.add_edge(source, target, weight=total, path=nodes_traversed)
    ...
    def build_level_dag_from_path(self):
        ...
        for curr_path_node in self.topo_path:
            if curr_path_node not in visited_nodes:
                if curr_path_node in solved_dags:
    ...
    def plot(self, save_path="", figsize=(10, 10)):
        ...
        edge_colors = [
            "red" if weight == float("inf") else "yellow" if weight > 0 else "green"
            for weight in edge_weights.values()
        ]

        # Plot the graph with edge colors
        plt.figure(figsize=figsize)
        nx.draw(self, pos, with_labels=True, arrows=True, edge_color=edge_colors)
        if save_path:
