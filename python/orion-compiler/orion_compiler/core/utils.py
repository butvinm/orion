...
def get_mnist_datasets(data_dir, batch_size, test_samples=10000, seed=None):
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                str(e)
                + " Could not install MNIST dataset automatically "
        ...
        if test_samples < len(test_dataset):
            test_dataset, _ = random_split(
                test_dataset, [test_samples, len(test_dataset) - test_samples]
            )
...
def get_cifar_datasets(data_dir, batch_size, test_samples=10000, seed=None):
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                str(e)
                + " Could not install CIFAR-10 dataset automatically "
...
def train_on_mnist(
    ...
    train(model, train_loader, test_loader, epochs, lr, momentum, weight_decay, device, save_path)
        ...
        train_bar.set_postfix(
            {
                ...
                "Acc": f"{100.0 * correct / total:.3f}% ({correct}/{total})",
            }
        )
