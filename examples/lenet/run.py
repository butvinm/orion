"""Full FHE pipeline for LeNet on MNIST.

Compile -> keygen -> encrypt -> evaluate -> decrypt -> print MAE.

Usage: cd examples/lenet && python run.py
"""
...
def main():
    ...
    # 3. Compile
    # LeNet: Conv2d(1,32,5,s=2,p=2) + BN2d + Quad
    #      → Conv2d(32,64,5,s=2,p=2) + BN2d + Quad
    #      → Flatten → Linear(3136,512) + BN1d + Quad
    #      → Linear(512,10)
    # 9 levels suffice (input_level=7, 0 bootstraps needed)
